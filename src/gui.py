...
class GUI:
    def __init__(self, master, board):
        ...
        self.canvas = tk.Canvas(
            master, width=500, height=500)  # The canvas widget
        self.canvas.pack()  # Display the canvas
        self.button = tk.Button(master, text="Click me!",
            ...
            for j in range(6):
                ...
                if node.color == 1:
                    ...
                    oval = self.canvas.create_oval(
                        j*83+10, i*83+10, j*83+73, i*83+73, fill='white')
                elif node.color == 2:
                    # Create a black cell
                    oval = self.canvas.create_oval(
                        j*83+10, i*83+10, j*83+73, i*83+73, fill='black')
                else:
                    # Create an empty cell
                    oval = self.canvas.create_oval(
                        j*83+30, i*83+30, j*83+53, i*83+53, fill='black')
                # Bind the click event to the cell
                self.canvas.tag_bind(
                    oval, "<Button-1>", lambda e, x=i, y=j: self.on_oval_click(x, y))
